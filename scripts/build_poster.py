...
BACKGROUND = (
    ...
    "and flooding, but no open dataset records where they physically stand.\n"
...
METHODS_TAIL = (
    ...
    "wildfire, inside a FEMA regulatory floodplain, or at peak ground "
    "acceleration of 0.30 g or more."
)
...
RESULTS_BODY = (
    ...
    "Eighteen states holding half the fleet sit below 10% exposed. Ten states "
    "holding a third sit above 60%.\n"
...
def fit_height(txt: str, size_pt: float, width_in: float,
    ...
    it. Sizing these boxes by eye is what clipped five blocks in an earlier
    build, so every box is measured instead.
    """
    usable = width_in - 0.25
    cpl = max(int(usable / (0.52 * size_pt / 72.0)), 10)
    line_h = size_pt * spacing / 72.0
    total = 0.0
    for para in txt.split("\n"):
        total += max(1, -(-len(para) // cpl)) * line_h + space_after_pt / 72.0
    return total + 0.14
...
def main() -> None:
    ...
    set_text(find(slide, "Put your title here"), TITLE, 80, bold=True,
    ...
    nh = fit_height(BOUNDS, 21, bw - 0.45)
    py = by + bh + 0.26
    ...
    for i, (num, lab) in enumerate(STATS):
        x = rx + 0.05 + i * (sw + 0.08)
        new_text(slide, x, y, sw, 0.82, num, 48, bold=True,
                 colour=GREEN, align=PP_ALIGN.CENTER)
        new_text(slide, x, y + 0.78, sw, 0.62, lab, 19, colour=DGREY,
                 align=PP_ALIGN.CENTER)
    y += 1.48
    ...
    nh3 = fit_height(NULLS, 21, cw - 0.45)
